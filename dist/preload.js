...
window.addEventListener('DOMContentLoaded', () => {
    ...
    function setupVoiceInterface() {
        ...
        if (!document.createElement || !document.body)
            return;
        if (document.getElementById('agy-voice-container'))
            return;
        // 1. Inject Styles
    ...
    `;
        ...
        const serviceUrlInput = drawer.querySelector('#agy-service-url-input');
        // Restore cached service URL
        try {
            const savedUrl = localStorage.getItem('agy_voice_service_url');
            if (savedUrl && serviceUrlInput)
        ...
        function renderTranscriptLine(speaker, text) {
        ...
        function getVisibleChatHistory() {
            ...
            const chatRows = document.querySelectorAll('.interactive-item-container, .chat-item, .chat-row, .interactive-session .monaco-list-row, [data-role="user"], [data-role="assistant"], [data-role="model"]');
            if (chatRows.length > 0) {
                chatRows.forEach((row) => {
                    const el = row;
                    const isUser = el.matches('[data-role="user"], .interactive-request, .chat-request, .user-prompt') ||
            ...
            testWs.onmessage = (e) => {
                try {
                    if (typeof e.data === 'string') {
                        ...
                        logMsg('GW-TEST', `Received audio frame (${e.data.byteLength} bytes)`, 'success');
                    }
                }
                catch (_) { }
            };
        ...
        function startMicAudioPipeline() {
            ...
            micProcessor.onaudioprocess = (e) => {
                ...
                if (drawer.style.display === 'flex' && meterInner) {
                    ...
                    meterInner.style.width = pct + '%';
                    if (meterVal)
                        meterVal.textContent = pct + '% (Floor: ' + Math.round(adaptiveNoiseFloor * 400) + '%)';
                }
                const now = Date.now();
